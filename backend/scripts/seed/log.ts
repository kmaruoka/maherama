...
export async function seedLog(prisma: PrismaClient) {
  ...
  const data = [
    ...
    { message: '<user:3:テストユーザー3>が<diety:1:天照大御神>を祈願しました', type: 'normal', time: new Date(now.getTime() - 1000 * 60 * 60 * 1) },
    { message: '<user:1:テストユーザー1>が<diety:2:月読命>を祀りました', type: 'normal', time: new Date(now.getTime() - 1000 * 60 * 60 * 0.8) },
    ...
    { message: '<user:1:テストユーザー1>が<diety:1:天照大御神>を祈願しました', type: 'normal', time: new Date(now.getTime() - 1000 * 60 * 60 * 0.5) },
    { message: '<user:2:テストユーザー2>が<diety:2:月読命>を祀りました', type: 'normal', time: new Date(now.getTime() - 1000 * 60 * 60 * 0.4) },
    ...
    { message: '<user:2:テストユーザー2>が<diety:1:天照大御神>を祈願しました', type: 'normal', time: new Date(now.getTime() - 1000 * 60 * 60 * 0.1) },
    { message: '<user:3:テストユーザー3>が<diety:2:月読命>を祀りました', type: 'normal', time: now }
  ];
