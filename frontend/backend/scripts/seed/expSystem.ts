...
export async function getUserPrayDistance(
  ...
  // 基本距離 + 能力による追加距離
  const baseDistance = 100 + (user.level - 1) * 10;
  const totalDistance = baseDistance + rangeAbilityLevel;
  return totalDistance * subscriptionMultiplier;
}
...
export async function getUserWorshipCount(
  ...
  // 基本回数 + 能力による追加回数 + サブスクリプション追加回数
  const baseCount = 1 + Math.floor(user.level / 10);
  return baseCount + worshipAbilityLevel + subscriptionBoost;
}
